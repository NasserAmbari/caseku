        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
            ...
            <li class="nav-item menu-open">
                ...
                <li class="nav-item">
                    ...
                    <li class="nav-item">
                        <a href="{{ route('listOrderSource') }}" class="nav-link active">
...
<div class="row">
    <div class="col-12">
        <div class="card">
            ...
            <div class="card-header">
                <div class="row">
                    ...
                    <div class="col-xl-4 offset-xl-7 ">
                        <form style="margin-bottom:0!important;" action="{{ route('listOrderSource') }}">
                ...
                <table class="table table-head-fixed text-nowrap table-hover">
                    <thead>
                        <tr>
                            ...
                            <th style="width:20rem;">@sortablelink('order_source','Order Source')</th>
                            <th style="text-align:end;">Action</th>
                        </tr>
                    </thead>

                    <tbody>
                        @foreach($order_sources as $data)
                        <tr>
                            <td>{{ $data->id }}</td>
                            <td>{{ $data->order_source }}</td>
                            <td style="text-align:end;">
                                <a class="updateData" data-target="#updateModal" data-toggle="modal"
                                    data-attr="{{ route('getEditSourceOrderData', ['id' => $data->id]) }}"><i
            ...
            {{ $order_sources->appends(\Request::except('page'))->render() }}

        </div>
    </div>
</div>

<div class="modal fade" id="createModal" tabindex="-1" role="dialog" aria-labelledby="createOrderSource"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-dialog-centered modal-md" role="document">
        <div class="modal-content">

            <div class="modal-header">
                <h5 class="modal-title" id="createOrderSource">Add New Order Source</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>

            <form action="{{ route('storeSourceOrderData') }}" method="POST">
                <div class="modal-body">
                    <div class="form-group">
                        <label for="createNewOrderSouce">Order Source</label>
                        <input type="input" class="form-control" id="createNewOrderSouce" name="ordersource">
                    </div>
                </div>
...
<div class="modal fade" id="updateModal" tabindex="-1" role="dialog" aria-labelledby="updateSourceOrder"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-dialog-centered modal-md" role="document">
        <div class="modal-content">

            <div class="modal-header">
                <h5 class="modal-title" id="updateSourceOrder">Update Phone Brand</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>

            <form action="{{ route('updateSourceOrderData') }}" method="POST">
                @method('PATCH')
                <div class="modal-body">

                    <div class="form-group">
                        <label for="updateNewSourceOrder">Update Order Source</label>
                        <input type="input" class="form-control" id="updateNewSourceOrder" name="sourceorder">
                    </div>
...
<div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="deleteBrandPhone"
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-dialog-centered modal-sm" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('deleteSourceOrderData') }}" method="POST">
                @method('DELETE')
...
<script>
    ...
    const sourceOrder = document.querySelector('#updateNewSourceOrder');
    ...
    allUpdateButton.forEach((elm, idx) => {
        elm.addEventListener('click', () => {
            ...
            getEditData(url, (result) => {
                console.log(result);
                sourceOrder.value = result.order_source;
                id.value = result.id;
            });
        });
    });
